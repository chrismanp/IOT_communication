...
index_arr_global = [];
def frameprocessing(frame, prevframe, tracker, label, local_tracker, s, outputfile, bbox_data):
	global ntracker;
	...
	if not args["baseline"] and not (args["t_periodic"] and trackFrame >= args["period"]):
	...
	for i in range(0, ntracker):
		if not ok_arr[i] or args["baseline"] or (args["t_periodic"] and trackFrame >= args["period"]):
			...
			bbox_label = object_detection(s, frame, numberofframes-1, bbox_data);
...
if __name__ == "__main__":
	...
	s.connect(addr)
	bbox_label = object_detection(s, frame, 0, bbox_data);
	...
	label = ""
	white_clip = clip1.fl_image(lambda x: frameprocessing(x, prevframe, tracker, label, local_tracker, s, outputfile, bbox_data))
