...
def frameprocessing(frame, label, local_tracker, s, outputfile, bbox_data):
	...
	for i in range(0, ntracker):
		if not ok_arr[i] or args["baseline"] or (args["t_periodic"] and trackFrame >= args["period"]):
			...
			frame_to_send= copy.deepcopy(frame);
	...
	for i in range(0, ntracker):
		if ok_arr[i]:
			...
			if len(confidence_arr_global) != 0: 
				...
				#label = str(bbox_area);
				#classlabel = str(CLASSES[int(index_arr_global[i])])
				
				#if int(index_arr_global[i]) in category_index.keys():
				#	classlabel = category_index[int(index_arr_global[i])]['name']
				
				classlabel = str( CLASSES_2[int (index_arr_global[i])] )

				label =  classlabel + " - " +str(confidence_arr_global[i]) + "%";
...
if __name__ == "__main__":
	...
	if bbox_label.shape[0] == 1:
		#tracker[i].init(frame, (0,0,0,0))			
		pass;
	else:	
		for i in range(0, bbox_label.shape[0]):
			bbox = (bbox_label[i][0], bbox_label[i][1], bbox_label[i][2], bbox_label[i][3])
	...
	for image_path in TEST_IMAGE_PATHS:
		...
		frameprocessing(image_np, label, local_tracker,s, outputfile, bbox_data)
